...
import { Camera, Pencil, Play, Plus, Shuffle, Trash2 } from "lucide-react";
    ...
    <div className="flex gap-5 items-end flex-wrap md:flex-nowrap mb-7">
      ...
      <div className="flex flex-col gap-1.5 min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          <div className="text-sm font-bold text-text-faint tracking-widest">{t("playlist.kicker")}</div>
          <IconButton onClick={onDeleteClick} aria-label={t("playlist.delete")} title={t("playlist.delete")} size={28}>
            <Trash2 size={13} />
          </IconButton>
        </div>
        ...
        <div className="text-base text-text-dim">{t("playlist.songsCount", { count: songCount })}</div>
