...
export function PlayerBarControls({ size = "sm" }: PlayerBarControlsProps) {
  ...
  return (
    <div className={cn("flex items-center", big ? "gap-6" : "gap-4")}>
      <IconButton active={shuffle} onClick={toggleShuffle} size={big ? 30 : 26} aria-label="Shuffle">
        <Shuffle size={big ? 16 : 13} />
      </IconButton>
      <button type="button" onClick={() => void prev()} className="text-text cursor-pointer" aria-label="Previous">
      ...
      <button type="button" onClick={() => void next()} className="text-text cursor-pointer" aria-label="Next">
        <SkipForward size={big ? 22 : 16} fill="currentColor" />
      </button>
      <IconButton active={repeatMode !== "off"} onClick={cycleRepeat} size={big ? 30 : 26} aria-label="Repeat">
