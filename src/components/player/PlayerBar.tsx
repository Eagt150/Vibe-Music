  ...
  return (
    <div className="flex items-center gap-3 md:gap-5 px-3 md:px-6 py-2 border-t border-border bg-surface flex-shrink-0 relative z-20">
      ...
      <div className="flex items-center gap-2.5 md:flex-[0_1_220px] md:justify-end">
        <IconButton onClick={toggleQueue} title={t("queue.title")} aria-label={t("queue.title")} size={isMobile ? 28 : 34}>
          <Menu size={16} />
        </IconButton>
        <VolumeSlider value={volumeLevel} onChange={setVolume} />
        {!isMobile && (
          <IconButton onClick={openFullScreen} title={t("player.expand")} aria-label={t("player.expand")} size={34}>
            <Maximize2 size={15} />
          </IconButton>
        )}
      </div>
